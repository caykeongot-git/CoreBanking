...
namespace CoreBanking.WinUI.UI
{
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            this.Text = $"Core Banking - Logged in as: {Session.CurrentUser?.FullName ?? "Admin"}";
            this.Size = new Size(1366, 768);
            this.StartPosition = FormStartPosition.CenterScreen;
            this.WindowState = FormWindowState.Maximized;
            this.FormBorderStyle = FormBorderStyle.None; // Không viền hệ thống để tự vẽ
        ...
        private async void LoadDashboard()
        {
            ...
            try
            {
                // Tạo Scope mới để lấy UnitOfWork (vì MainForm sống lâu, DbContext cần ngắn hạn)
            ...
            // --- ROW 1: STAT CARDS (Màu sắc Banking) ---
            ...
            row1.Controls.Add(CreateStatCard("TOTAL ASSETS", totalAssets.ToString("C0"), "Customer Deposits", ThemeColor.SuccessColor), 0, 0); // Xanh lá
            row1.Controls.Add(CreateStatCard("CUSTOMERS", totalCustomers.ToString(), "Active Users", ThemeColor.PrimaryColor), 1, 0);       // Xanh dương
            row1.Controls.Add(CreateStatCard("TOTAL LOANS", totalLoans.ToString(), "All Applications", ThemeColor.WarningColor), 2, 0);     // Vàng
            row1.Controls.Add(CreateStatCard("PENDING", pendingLoans.ToString(), "Need Approval", ThemeColor.DangerColor), 3, 0);           // Đỏ
            ...
            Panel pnlTrend = CreateShadowPanel(ThemeColor.PrimaryColor); // Nền xanh Banking
            ...
            chartPie.Series = new SeriesCollection {
                new PieSeries { Title = "Personal", Values = new ChartValues<double> { 45 }, Fill = new System.Windows.Media.SolidColorBrush(ToMediaColor(ThemeColor.PrimaryColor)), DataLabels = false },
                new PieSeries { Title = "Business", Values = new ChartValues<double> { 35 }, Fill = new System.Windows.Media.SolidColorBrush(ToMediaColor(ThemeColor.WarningColor)), DataLabels = false },
                new PieSeries { Title = "Mortgage", Values = new ChartValues<double> { 20 }, Fill = new System.Windows.Media.SolidColorBrush(ToMediaColor(ThemeColor.SuccessColor)), DataLabels = false }
            };
            ...
            flow.Controls.Add(CreateProgressItem("Excellent (750+)", 30, ThemeColor.SuccessColor));
            flow.Controls.Add(CreateProgressItem("Good (650-749)", 50, ThemeColor.PrimaryColor));
            flow.Controls.Add(CreateProgressItem("Fair (600-649)", 15, ThemeColor.WarningColor));
            flow.Controls.Add(CreateProgressItem("Poor (<600)", 5, ThemeColor.DangerColor));
            ...
            dgv.DefaultCellStyle = new DataGridViewCellStyle { SelectionBackColor = ThemeColor.PrimaryColor, SelectionForeColor = Color.White, Font = new Font("Segoe UI", 9), Padding = new Padding(10, 0, 0, 0), ForeColor = Color.FromArgb(64, 64, 64) };
            ...
            loanLayout.Controls.Add(CreateStatusBox("Active", loanActive.ToString(), ThemeColor.PrimaryColor));
            loanLayout.Controls.Add(CreateStatusBox("Pending", loanPending.ToString(), ThemeColor.WarningColor));
            loanLayout.Controls.Add(CreateStatusBox("Paid Off", loanPaid.ToString(), ThemeColor.SuccessColor));
            loanLayout.Controls.Add(CreateStatusBox("Bad Debt", loanBadDebt.ToString(), ThemeColor.DangerColor));
            ...
            flowTasks.Controls.Add(CreateTaskItem("High Value Transaction > $10k", "5 mins ago", ThemeColor.DangerColor));
            flowTasks.Controls.Add(CreateTaskItem("New Loan Request: Nguyen Van A", "15 mins ago", ThemeColor.WarningColor));
            flowTasks.Controls.Add(CreateTaskItem("Daily Backup Completed", "1 hour ago", ThemeColor.SuccessColor));
        ...
        private void SetupSidebar()
        {
            ...
            // Logo
            var pnlLogo = new Panel { Dock = DockStyle.Top, Height = 60, BackColor = ThemeColor.PrimaryColor };
            ...
            Button btnLogout = new Button
            {
                ...
                ForeColor = ThemeColor.DangerColor,
        ...
        {
            _pnlHeader = new Panel { Dock = DockStyle.Top, Height = 60, BackColor = ThemeColor.PrimaryColor };
            _btnMenuToggle = new Button { Text = "☰", Font = new Font("Segoe UI", 18, FontStyle.Regular), ForeColor = Color.White, BackColor = Color.Transparent, FlatStyle = FlatStyle.Flat, Size = new Size(50, 60), Dock = DockStyle.Left, Cursor = Cursors.Hand };
            _btnMenuToggle.FlatAppearance.BorderSize = 0;
            _btnMenuToggle.Click += (s, e) => ToggleSidebar();

            Label lblTitle = new Label { Text = "Dashboard Overview", ForeColor = Color.White, Font = new Font("Segoe UI", 14, FontStyle.Regular), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft, Dock = DockStyle.Fill, Padding = new Padding(10, 0, 0, 0) };
            _lblPageTitle = lblTitle;

            // Window Controls (Custom Close/Min)
            Label lblClose = new Label { Text = "✕", ForeColor = Color.White, Font = new Font("Arial", 14), AutoSize = true, Cursor = Cursors.Hand, Location = new Point(this.Width - 40, 15), Anchor = AnchorStyles.Top | AnchorStyles.Right };
            ...
            this.Controls.Add(_pnlHeader); _pnlHeader.BringToFront();
        ...
        private void AddSidebarButton(string icon, string text, bool isActive, EventHandler onClick)
        {
            Button btn = new Button
            {
                ...
                ForeColor = isActive ? ThemeColor.PrimaryColor : Color.Gray,
        ...
        private System.Windows.Media.Color ToMediaColor(System.Drawing.Color c) { return System.Windows.Media.Color.FromRgb(c.R, c.G, c.B); }
    ...
    // --- HELPER CONTROL: FLAT PROGRESS BAR (Để code tự chạy được luôn) ---
    public class FlatProgressBar : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Value > 0)
            {
                ...
                using (SolidBrush barBrush = new SolidBrush(ProgressColor))
                {
